...
def sanitize_filename(filename):
    invalid_char_map = {
        ...
        "|": "-",  # Replace any other invalid characters as needed
        "<": "",
        ">": "",
        "\"": "",
        "/": "",
    ...
    for invalid_char, replacement in invalid_char_map.items():
...
def model_test(runid, engine, dataloader, device, logger, cfg, mode='Test'):
    ...
    # Format the filename with metrics and sanitize it
        name = 'exp{:d}_Test_mae_{:.4f}_mape_{:.4f}_rmse_{:.4f}.npy'.format(cfg['expid'], mtest_mae, mtest_mape, mtest_rmse)
        safe_name = sanitize_filename(name)
        path = os.path.join(path_save_pred, safe_name)
...
def baseline_test(runid, model, dataloader, device, logger, cfg):
    ...
    if torch.cuda.is_available():
        device = torch.device("cuda:0")  # Ensuring we use the first GPU
        save_dict = torch.load(best_mode_path, map_location=device)
    else:
        save_dict = torch.load(best_mode_path, map_location=torch.device('cpu'))
